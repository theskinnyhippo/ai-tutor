...
export const DashboardTrial = () => {
    ...
    const trpc = useTRPC();
    const { data } = useQuery(trpc.premium.getFreeUage.queryOptions())
    ...
    return (
        <div className="border border-border/10 rounded-lg w-full bg-white/5 flex flex-col gap-y-2 ">
            ...
            <Button asChild className="bg-transparent border-t border-border/10 hover:bg-white/55 rounded-t-none">
                <Link href="/upgrade">
                    Upgrade
                </Link>
            </Button>
